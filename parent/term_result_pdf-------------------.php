...
$iStudent=$db->getRow("select * from manage_student where student_id='".$_GET['student_id']."'");
...
$iManageStudentId=$db->getRows("select id from manage_student where student_id = '" .$iStudent['student_id']. "' ");
...
$scoresTotal=$db->getRow("select * from input_score_class_teacher where  session_id = '" .$_GET['session']. "' and term_id = '".$_GET['term_id']."' and assesment_id='".$Val."' and student_id IN ($iCommaSeparatedStudent) and subject_id='".$IlistTotal['id']."' and create_by_userid='".$iParent['create_by_userid']."'");
...
$iTotalStudents = $db->getVal("select count(*) from manage_student where  create_by_userid='".$create_by_userid."' and session='".$_GET['session']."' and term_id='".$_GET['term_id']."'");
?>
<html>
<style>
   #watermark {
				...
				<table style="width:100%;">
					<tr>
					  ...
					  <?php	$ggrade=$grandTotalsss/$tSubtotal; ?>
						<td style=""><?php echo $graddingg=$db->getVal("select grade from school_grade where create_by_userid='".$iParent['create_by_userid']."' and maximum_number > ".$ggrade." or maximum_number = ".$ggrade."");?> </td>
                           <?php } ?>
...
$scoreNew=$db->getRow("select * from input_score_class_teacher where assesment_id IN($assesmentIn) and student_id='".$iStudent['id']."'  and create_by_userid='".$iParent['create_by_userid']."'");
	...
	<tbody style="font-size: 18px;font-weight: bolder;font-family: sans-serif;">
		...
		<tr style="height: 35px;">
			...
			foreach($totalAssesment as $Val) 
			{ 	$count=$count+1;
				$scores=$db->getRow("select * from input_score_class_teacher where  term_id ='".$_GET['term_id']."'  and session_id ='".$_GET['session']."'  and assesment_id='".$Val."' and student_id IN ($iCommaSeparatedStudent) and subject_id='".$Ilist['id']."' and create_by_userid='".$iParent['create_by_userid']."'");
				?>
			...
			<td style="font-weight: 100; text-align: center;  border: 1px solid #000000;">
			<?php echo $gradding=$db->getVal("select grade from school_grade where  create_by_userid='".$iParent['create_by_userid']."' and maximum_number > ".$grandTotal." or maximum_number = ".$grandTotal."");
			?>
			</td>
			...
			<td style="font-weight: 100; text-align: center;  border: 1px solid #000000;">
            <?php echo $db->getVal("select description from school_grade where create_by_userid='".$iParent['create_by_userid']."' and maximum_number > ".$grandTotal." or maximum_number = ".$grandTotal."");?>   
			</td>
		</tr>
		<?php } ?>
	</tbody>
			...
			<td style="width:100%;" >
				<table style="width:100%;">
					...
					<tr>
						...
						<?php $sign_term=$db->getRow("select nextTerm,sign from  principal_sign_nextTerm where session_id='".$_GET['session']."' and term_id='".$_GET['term_id']."'and class_id='".$iStudent['class']."'  and create_by_userid='".$iParent['create_by_userid']."'"); ?>
						<td style="width: 90px;">
							<img  src="../uploads/<?php echo $sign_term['sign']; ?>" style="width: 70px;">
						</td>
					</tr>
					<tr>
