...
/**
 * ============================================================================
 * PARENT TERM RESULT PDF - IDENTICAL TO SKOOL VERSION
 * ============================================================================
 */
...
// Start session for parent authentication
...
if (empty($_SESSION['parent_id'])) {
    die("Access denied. Please login as parent.");
}

$parentId = $_SESSION['parent_id'];
$sessionSchoolId = (int)($_SESSION['create_by_userid'] ?? 0);

// Build compatible parent-id set (current + legacy aliases) for access checks.
$parentLookupIds = [];
$pushParentId = function ($value) use (&$parentLookupIds) {
    $v = trim((string)$value);
    if ($v !== '') {
        $parentLookupIds[$v] = true;
    }
};
...
if ($sessionUserId > 0) {
    $sessionGuardian = db_get_row("SELECT parent_id, student_id_str FROM student_guardian WHERE id = ?", [$sessionUserId]);
    if (is_array($sessionGuardian)) {
        $pushParentId($sessionGuardian['parent_id'] ?? '');
        $pushParentId($sessionGuardian['student_id_str'] ?? '');
    }
}

$linkedGuardians = db_get_rows(
    "SELECT parent_id, student_id_str FROM student_guardian WHERE type = 1 AND (parent_id = ? OR student_id_str = ?)",
    [$parentId, $parentId]
) ?: [];

foreach ($linkedGuardians as $g) {
    $pushParentId($g['parent_id'] ?? '');
    $pushParentId($g['student_id_str'] ?? '');
}

$parentLookupIds = array_values(array_keys($parentLookupIds));
if (empty($parentLookupIds)) {
    $parentLookupIds = [$parentId];
}
$parentInClause = implode(',', array_fill(0, count($parentLookupIds), '?'));
...
// ============================================================================
// GET STUDENT - VERIFY PARENT OWNS THIS STUDENT
// ============================================================================
$iStudent = [];

// FIRST: Try by randomid (most reliable)
if ($requestedRandomId !== '') {
    $studentQuery = "SELECT * FROM manage_student WHERE randomid = ? AND parent_id IN ($parentInClause)";
    $studentParams = array_merge([$requestedRandomId], $parentLookupIds);
...
// SECOND: Try by database ID
if (empty($iStudent)) {
    $studentDbId = (int)($_GET['student_db_id'] ?? 0);
    if ($studentDbId > 0) {
        $studentQuery = "SELECT * FROM manage_student WHERE id = ? AND parent_id IN ($parentInClause)";
        $studentParams = array_merge([$studentDbId], $parentLookupIds);
        ...
        $iStudent = db_get_row($studentQuery, $studentParams);
    }
}

// THIRD: Try by student_id
if (empty($iStudent) && $requestedStudentId !== '') {
    $studentQuery = "SELECT * FROM manage_student WHERE student_id = ? AND parent_id IN ($parentInClause)";
    $studentParams = array_merge([$requestedStudentId], $parentLookupIds);
...
// FOURTH: Try by student_id with session and term
...
$forceA3Paper = in_array($paperMode, ['a3', 'a3_portrait'], true);
$forceCompactForSchoolIds = in_array((int)$SCHOOL_ID, [30, 64], true);
$assessmentCountForLayout = max(1, count($totalAssesment));
$denseLayout = $forceCompactForSchoolIds || ($subjectCountForLayout >= 10) || ($assessmentCountForLayout >= 4 && $columnCount >= 13) || ($columnCount >= 14);
$useA3Page = $forceA3Paper || ($autoPaperMode && $denseLayout);
$paperSize = $useA3Page ? 'A3' : 'A4';
$compactMode = $denseLayout || $forceCompactForSchoolIds;
$traitsMarginTop = $compactMode ? '25px' : '45px';
        ...
        <table class="subjects-table">
            ...
            <colgroup>
                ...
                <?php foreach ($totalAssesment as $Val): ?>
                    <col style="width:5%;"><?php endforeach; ?>
                <col style="width:6%;">
                <col style="width:5%;">
                <?php if ($showPos): ?>
