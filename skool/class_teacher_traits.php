...
/**
 * ============================================================================
 * CLASS TEACHER - AFFECTIVE TRAITS (MODERN UI)
 * ============================================================================
 * - Admin sees ALL classes (usertype = 1 or 0)
...
$PageTitle = "Affective Traits";
$FileName = 'class_teacher_traits.php';
...
// ============================================================================
// GET SELECTED TRAIT DATA
// ============================================================================
$selectedTraitData = [];
if (!empty($selectedTrait)) {
    $selectedTraitData = db_get_row(
        "SELECT * FROM manage_traits WHERE randomid = ? AND create_by_userid = ?",
        [$selectedTrait, $create_by_userid]
    );
}
...
// ============================================================================
// GET CLASSES - BASED ON USER TYPE (Admin sees ALL, Teacher sees assigned)
// ============================================================================
$classes = [];
...
// ============================================================================
// GET SESSIONS, TERMS, TRAITS FOR FILTERS
// ============================================================================
...
$traits = db_get_rows(
    "SELECT * FROM manage_traits WHERE create_by_userid = ? ORDER BY id ASC",
    [$create_by_userid]
);
...
if (!empty($selectedClass['id']) && !empty($selectedSession) && !empty($selectedTerm) && !empty($selectedTraitData['id'])) {
    ...
    if (!empty($students)) {
        foreach ($students as $student) {
            $rating = db_get_val(
                "SELECT trait FROM student_traits_class_teacher 
                 WHERE session_id = ? AND term_id = ? AND class_id = ? 
                 AND traits_id = ? AND student_id = ? AND create_by_userid = ?",
                [$selectedSession, $selectedTerm, $selectedClass['id'], $selectedTraitData['id'], $student['id'], $create_by_userid]
            );
...
if (isset($_POST['save_ratings'])) {
    ...
    // Delete existing ratings for this combination
    db_delete(
        "student_traits_class_teacher",
        "session_id = ? AND term_id = ? AND class_id = ? AND traits_id = ? AND create_by_userid = ?",
        [$selectedSession, $selectedTerm, $selectedClass['id'], $selectedTraitData['id'], $create_by_userid]
    );
    ...
    foreach ($studentIds as $studentId) {
        ...
        if (!empty($rating)) {
            $result = db_insert("student_traits_class_teacher", [
                'session_id' => $selectedSession,
                'term_id' => $selectedTerm,
                'class_id' => $selectedClass['id'],
                'traits_id' => $selectedTraitData['id'],
    ...
    $redirectUrl = $FileName . "?action=input_score&randomid=" . urlencode($selectedClassRandomid) . "&session=" . urlencode($selectedSession) . "&term_id=" . urlencode($selectedTerm) . "&trait=" . urlencode($selectedTrait);
            ...
            <div class="content">
                <div class="traits-container">
                    ...
                    <div class="page-header">
                        <h2><i class="fa fa-heart"></i> <?= htmlspecialchars($PageTitle) ?></h2>
                        <p>Rate students on affective traits (Honesty, Punctuality, Neatness, etc.)</p>
                    </div>
                    ...
                    <div class="two-column-layout">
                            ...
                            <div class="class-list">
                                <?php if (!empty($classes)): ?>
                                    <?php foreach ($classes as $class): ?>
                                        <a href="?action=input_score&randomid=<?= urlencode($class['randomid']) ?><?= $selectedSession ? '&session=' . urlencode($selectedSession) : '' ?><?= $selectedTerm ? '&term_id=' . urlencode($selectedTerm) : '' ?><?= $selectedTrait ? '&trait=' . urlencode($selectedTrait) : '' ?>"
                        ...
                        <div class="ratings-panel">
                            ...
                            <?php if (!empty($selectedClass)): ?>
                                ...
                                <div class="filter-card">
                                    <form method="GET" action="" id="filterForm">
                                        ...
                                        <div class="filter-grid">
                                            ...
                                            <div class="filter-group">
                                                <label><i class="fa fa-heart"></i> Trait</label>
                                                <select name="trait" class="filter-select" onchange="this.form.submit()">
                                                    <option value="">-- Select --</option>
                                                    <?php foreach ($traits as $trait): ?>
                                                        <option value="<?= $trait['randomid'] ?>" <?= ($selectedTrait == $trait['randomid']) ? 'selected' : '' ?>>
                                ...
                                <?php if (!empty($selectedSession) && !empty($selectedTerm) && !empty($selectedTraitData)): ?>
                                ...
                                <?php elseif (!empty($selectedSession) && !empty($selectedTerm) && empty($selectedTrait)): ?>
                                    <div class="alert alert-info">
                                        <i class="fa fa-heart"></i>
                                        <div>Please select an affective trait to continue.</div>
                                    </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            selects.forEach(function(select) {
                select.addEventListener('change', function() {
                    ...
                    var trait = filterForm.querySelector('select[name="trait"]');
                    if (session && term && trait && session.value && term.value && trait.value) {
                        filterForm.submit();
                    }
                });
            });
