...
$aryList=$db->getRow("select * from school_subject  where randomid='".$_GET['randomid']."' and create_by_userid='".$create_by_userid."'");
$PageTitle="Input Score For All Cas";
$FileName = 'input_score_class_teacher_for_all_cas.php';
...
<div class="zasw1">
						...
						<div class="card-box">
                                <table  class="table table-striped table-bordered">
                                    ...
                                    <tbody>
                                    <?php $i=0;
									$aryList11=$db->getRows("select * from input_score_subject_teacher  where class_id='".$aryList['class_id']."' and create_by_userid='".$create_by_userid."'");
						       ...
						       foreach($aryList11 as $iList)
							{
							 ...
							 ?>
                                <tr>
									  <td><?php echo $db->getVal("select student_id from manage_student where id='".$iList['student_id']."'"); ?>
									  </td>
									  <input type="hidden" value="<?php echo $iList['student_id']; ?>" name="student_id">
									  <input type="hidden" value="<?php echo $iList['subject_id']; ?>" name="subject_id">
									  <td><?php echo $db->getVal("select first_name from manage_student where id='".$iList['student_id']."'"); ?>
									  </td>
									 <td><?php echo $db->getVal("select last_name from manage_student where id='".$iList['student_id']."'"); ?>
									 </td>
								     <td><?php echo $db->getVal("select other_name from manage_student where id='".$iList['student_id']."'"); ?></td>
									...
									<td><textarea></textarea>
									     <button type="submit" name="submitcomment"  >Insert Template</button>
								    </td>
