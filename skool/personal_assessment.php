...
if(isset($_POST['addnewrecord']))
{
    ...
    if($validate->validate() && count($stat) == 0)
    {
				...
				$iLastId=$db->getVal("select id from personal_assessment order by id desc")+1;		
		...
		$aryData = array(
				...
				$flgIn = $db->insertAry("personal_assessment", $aryData);		
	...
	$validate->addRule($_POST['assessment'],'','Assessment',true);
     if($validate->validate() && count($stat) == 0)
		...
		$aryData = array(
			...
			$flgIn2 = $db->updateAry("personal_assessment", $aryData, "where randomid='".$_GET['randomid']."'");
...
{
	$flgIn1 = $db->delete("personal_assessment", "where randomid='".$_GET['randomid']."'");
