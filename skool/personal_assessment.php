...
/**
 * ============================================================================
 * PERSONAL ASSESSMENT - MODERN REDESIGN
 * ============================================================================
 * Description: Manage personal assessment types with CRUD operations
...
$PageTitle = "Personal Assessment";
$FileName = 'personal_assessment.php';
...
if (isset($_POST['addnewrecord'])) {
	...
	if ($validate->validate() && count($stat) == 0) {
		$iLastId = db_get_val("SELECT id FROM personal_assessment ORDER BY id DESC") + 1;
		...
		$flgIn = db_insert("personal_assessment", $aryData);

		if ($flgIn) {
			$_SESSION['success'] = "Personal assessment saved successfully.";
			redirect($FileName);
		} else {
			$stat['error'] = "Failed to save personal assessment. Please try again.";
		}
	} else {
...
if (isset($_POST['updaterecord']) && !empty($_GET['randomid'])) {
	...
	if ($validate->validate() && count($stat) == 0) {
		...
		$flgIn = db_update("personal_assessment", $aryData, "randomid = ?", [$_GET['randomid']]);

		if ($flgIn !== false) {
			$_SESSION['success'] = "Personal assessment updated successfully.";
			redirect($FileName);
		} else {
			$stat['error'] = "Failed to update personal assessment. Please try again.";
		}
	} else {
		$stat['error'] = $validate->errors();
	}
...
// ============================================================================
// DELETE PERSONAL ASSESSMENT
// ============================================================================
if (isset($_GET['action']) && $_GET['action'] == 'delete_mas' && !empty($_GET['randomid'])) {
	$flgIn = db_delete("personal_assessment", "randomid = ?", [$_GET['randomid']]);
	if ($flgIn !== false) {
		$_SESSION['success'] = 'Personal assessment deleted successfully.';
	} else {
		$_SESSION['error'] = 'Failed to delete personal assessment.';
	}
	redirect($FileName);
}
...
if (!empty($_GET['randomid'])) {
	$editRecord = db_get_row(
		"SELECT * FROM personal_assessment WHERE randomid = ? AND create_by_userid = ?",
...
<body class="fixed-left">
			...
			<div class="content">
				<div class="assessment-container">
					...
					<div class="page-header">
						<h2><i class="fa fa-user-plus"></i> <?= htmlspecialchars($PageTitle) ?></h2>
						<p>Create and manage personal assessment types</p>
					</div>
					...
					<div class="card">
						<div class="card-header">
							...
							<h3><?= (!empty($editRecord)) ? 'Edit Personal Assessment' : 'Add New Personal Assessment' ?></h3>
						</div>
						<div class="card-body">
							<form method="POST" action="<?= (!empty($editRecord)) ? '?randomid=' . urlencode($editRecord['randomid']) : '' ?>" id="assessmentForm">
								<div class="form-group">
									...
									<input type="text" name="assessment" placeholder="Enter assessment name..."
										value="<?= htmlspecialchars($editRecord['assessment'] ?? $_POST['assessment'] ?? '') ?>" required autofocus>
								</div>
					...
					<div class="card">
						<div class="card-header">
							...
							<h3>Personal Assessment List</h3>
						...
						<div class="card-body">
							...
							<?php else: ?>
								<div class="empty-state">
									...
									<h4>No Personal Assessments Found</h4>
									<p>Add your first personal assessment using the form above.</p>
								</div>
							<?php endif; ?>
						</div>
					</div>
	...
	<script>
		function confirmDelete(url) {
			if (confirm('Are you sure you want to delete this personal assessment? This action cannot be undone.')) {
