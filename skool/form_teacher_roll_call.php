...
$pageTitle = 'Manage  Roll Call';
$Filename = 'form_teacher_roll_call.php';
if(isset($_POST['submit']))
{
	...
	{    
     $arydetail = $db->getRow("select * from form_teacher_roll_call where rollCall_date='" .$_POST['roll_call_date']. "' and class_id='" .$_POST['class']. "' and session_id='" .$_POST['session']. "'");
        if($arydetail['id']=='')
		{
        ...
        $flgIn2 = $db->insertAry("form_teacher_roll_call",$aryData);
...
<div id="wrapper">
                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <?php } elseif ($_GET['action'] == 'edit') {
                            $arydetail = $db->getRow("select * from  form_teacher_roll_call where randomid='" .$_GET['randomid']. "' "); ?>
  ...
  $iattand=$db->getRow("select * from   form_teacher_roll_call where class_id='".$arydetail['class_id']."' and rollCall_date='".$arydetail['rollCall_date']."'and randomid='".$_GET['randomid']."'");
								?>	
...
value="<?php echo $istudent['student_id'];?>"<?php if(in_array($istudent['student_id'],$itotalPresent)){echo "checked";}?> />
									...
									<td>
									<?php $latePresent=$iattand['late_student'];
									$latePresentStudent=explode(',',$latePresent);
									
									?>
                                <input type="checkbox" name="is_late[]"  value="<?php echo $istudent['student_id'];?>"
			<?php if(in_array($istudent['student_id'],$latePresentStudent)){echo "checked";}?> 
								onchange="getPresent(<?php echo $istudent['student_id'];?>)"/> 
		            ...
		            <?php } ?>
                                        ...
                                        <button type="submit" name="edit" style="color:white;" class="btn btn-default">Update</button>
                                        <a href="<?php echo $Filename; ?>" style="color:white;" class="btn btn-default">Back</a>
                                    </section>
                                </div>
                                </form>
                        ...
                        <?php } else { ?>
                            <div class="card-box">
                                <table id="datatable" class="table table-striped table-bordered">
                                    ...
                                    <tbody>
                                    <?php
                                    $aryList = $db->getRows("select * from form_teacher_roll_call order by id desc");
...
function getPresent(student_id) {
  ...
  if (student_id!='') {
	...
	document.getElementById("uid_"+"student_id").checked;
  } else {
