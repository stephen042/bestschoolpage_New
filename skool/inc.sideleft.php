...
if ($schoolOwnerId <= 0) {
    $schoolOwnerId = isset($create_by_userid) ? (int)$create_by_userid : (int)($_SESSION['userid'] ?? 0);
}
...
if (!$isAdminMenu) {
    ...
    // Get staff role assignment. Some records are stored with school_register.id,
    // others with staff_manage.id, so match both safely.
    $iStaffCheck = [];
    $candidateStaffIds = [];
    if ($sessionUserId > 0) {
...
// Sidebar logo image (no school name text)
