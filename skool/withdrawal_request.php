...
$iCurrentUserWalletAmount=$db->getRow("select walletamount from  school_register where id='".$_SESSION['userid']."'");
if(isset($_POST['addnewrecore']))
{
    $validate->addRule($_POST['amount'],'','Amount',true);

    if($validate->validate() && count($stat) == 0)
    {
		
	$iAlreadyPending = $db->getVal("select id from withdrawal_request where request_by='".$_SESSION['userid']."' and status = '0'");	
	if($iAlreadyPending=='')
		{	
		
		if($_POST['amount']<=$iCurrentUserWalletAmount['walletamount'])
			{
        $iLastId=$db->getVal("select id from withdrawal_request order by id desc")+1;
        ...
        $aryData = array(
				...
				$flgIn = $db->insertAry("withdrawal_request", $aryData);
				$_SESSION['success']="withdrawal request have been sent successfully.";
				redirect($FileName);
			} else {
				
				$stat['error']='Your dont have sufficient fund in your account';	
			}
			
			
			} else {
				
				$stat['error']='Your already have 1 pending request.';	
			}
	}
    else
	{
              ...
              <div class="col-md-12"> 
                ...
                <div class="card">
                  <ul class="nav nav-tabs" role="tablist">
                    <li role="presentation" class="<?php if($_GET['action']=='' ) { echo "active"; } ?>"> <a href="<?php echo $Filename; ?>?action=verification_status"> <i class="fa fa-line-chart" aria-hidden="true"></i><span>Withdrawal Request</span> </a> </li>
                    <li role="presentation" class="<?php if($_GET['action']=='add') { echo "active"; } ?>"> <a href="<?php echo $Filename; ?>?action=add"> <i class="fa fa-line-chart" aria-hidden="true"></i><span>Create New Request</span> </a> </li>
                    <li role="presentation" > <a href="#"> You have <?php echo $iCurrentUserWalletAmount['walletamount']; ?> in your wallet </a> </li>
                  </ul>
                  <div class="tab-content">
                    ...
                    <form action="" method="POST">
                      <div class="form-group clearfix plims">
                        <div class="col-lg-12">
                          <input autocomplete="off" name="amount" class="form-control" value="" placeholder="Enter Amount To Withdrawal " type="text">
                        </div>
             
                        <div class="col-lg-12 sgot">
                          <div class="row">
                            <div class="savdtls">
                              <button type="submit" name="addnewrecore" class="btn btn-default">Submit</button>
                            </div>
                          </div>
                        </div>
                      </div>
                ...
                <div class="abhish">
                  <div class="card-box">
                    <table class="table table-striped table-bordered">
                      ...
                      <tbody>
                        <?php $i=0;
							$aryList = $db->getRows("select * from withdrawal_request where request_by='".$_SESSION['userid']."'");
							foreach($aryList as $iList) 
							{ 
							$i=$i+1;
							
							$iRoleName=$db->getRow("select username from school_register where id='".$iList['request_by']."' ");
     						
							?>
                        <tr>
                          <td><?php echo $i ?></td>
                          <td><?php echo $iList['amount'];?></td>
                          <td><?php echo $iRoleName['username'];?></td>
                          <td><?php echo $iList['create_at'];?></td>
                          <td><?php if($iList['status']=='1') { echo 'Amount Transfered'; } elseif($iList['status']=='0') { echo 'Pending'; } else { echo 'Reject'; } ?></td>
                        </tr>
                        <?php } ?>
                      </tbody>
                    </table>
