...
if($_GET['term_id'] == 'all')
{  
  $iStudent=$db->getRows("select * from manage_student where class='".$_GET['class_id']."' and student_id = '".$_GET['student_id']."'");  
}
else{
  $iStudent=$db->getRow("select * from manage_student where randomid='".$_GET['randomid']."' and class='".$_GET['class_id']."' and term_id='".$_GET['term_id']."'");
}
...
$subjectdetails=$db->getRows("select id from school_subject where class_id = '" .$iStudent['class']. "' and create_by_userid='".$create_by_userid."'");
foreach ($subjectdetails as $Ilists) { 
$aiiisub=$Ilists['id'];
$i=0;
$tStuden=0;
$aryList11=$db->getRows("select id from manage_student  where class='".$iClass['id']."' and session='".$_GET['session']."' and term_id='".$_GET['term_id']."'  and create_by_userid='".$create_by_userid."' order by first_name asc");
foreach($aryList11 as $iList)
{
	$tStuden=$tStuden+1;
foreach($totalAssesment as $Val) 
{ 
$score=$db->getRow("select score from input_score_class_teacher where assesment_id='".$Val."' and student_id='".$iList['id']."' and subject_id='".$Ilists['id']."' and create_by_userid='".$create_by_userid."'");
...
<table style="width:100%;">
  <tr>
    <td colspan="2" style="width:250px;padding-top:80px;"><div>&nbsp;&nbsp;&nbsp; <img style="width:250px" src="<?php echo 'https://'.$_SERVER['HTTP_HOST'].'/uploads/'.$iSchool['logo']; ?>"> </div></td>
      ...
      <td style="width:100%;" ><table style="width:100%;">
          ...
          <tr>
            ...
            <td style=""><?php 
						...
						$totalPressent= $db->getRows("select present from class_teacher_roll_call where  session_id='".$_GET['session']."' and term_id='".$_GET['term_id']."' and class_id='".$iStudent['class']."' and create_by_userid='".$create_by_userid."'");
