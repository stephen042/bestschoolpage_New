...
function sms($number,$message,$skname)
{
...
$iUrlMessage  = ($url);
$ret = file_get_contents($iUrlMessage);
return $ret;
}

if(isset($_POST['parentSMS']))
	{
	$iCheckSmsm=$db->getVal("select id from sms_payment where create_by_userid='".$create_by_userid."' and no_of_sms >0" );
if($iCheckSmsm!=''){
	$validate->addRule($_POST['pSMS'],'','Message',true);	
    if ($validate->validate() && count($stat) == 0)
	{	
	 $mainMessagee=$_POST['pSMS'];
foreach($_POST['sendSMSToParent'] as $key=>$valu)
		{
		$number=$valu;
		$message=$mainMessagee;
        $getContain=sms($number,$message,$skoolName);	
		$flgUp = $db->update("update sms_payment set no_of_sms= no_of_sms - 1 where create_by_userid='".$create_by_userid."'" );
...
if($iCheckSmsm!=''){
	...
	foreach($_POST['sendSsmsToStaff'] as $key=>$val)
			{
				 $number=$val;
				 $message=$mainMessage;
	  $getContain=sms($number,$message,$skoolName);	
            ...
            <div class="col-md-12"> 
              ...
              <div class="card">
                <ul class="nav nav-tabs" role="tablist">
                  <li role="presentation" class="<?php if($_GET['action']=='' || $_GET['action']=='parents') { echo "active"; } ?>"> <a href="<?php echo $Filename; ?>?action=parents"> <i class="fa fa-exclamation-circle" aria-hidden="true"></i> <span>Parents</span> </a> </li>
                  <li role="presentation" class="<?php if($_GET['action']=='staff') { echo "active"; } ?>"> <a href="<?php echo $FileName; ?>?action=staff"> <i class="fa fa-users" aria-hidden="true"></i> <span>Staff</span> </a> </li>
                </ul>
                  ...
                  <div role="tabpanel" class="tab-pane active" id="home">
                    <div class="ab-1">
                      <form action="" method="POST">
                        ...
                        Message
                        <textarea class="form-control" name="pSMS" placeholder="Type your message here..."><?php echo $skoolname['name']; ?></textarea>
                       
                        <button type="submit" name="parentSMS"style="float:left;">SEND SMS </button>
                       	</div>
				...
				$aryList=$db->getRows("select DISTINCT parent_id from student_guardian where create_by_userid='".$create_by_userid."'");
						foreach($aryList as $iNeList)
							{	$i=$i+1;
							 ...
							 ?>
                            <tr>
                              <td><input type="checkbox"  name="sendSMSToParent[]" style="width:15px;height:15px"  value="<?php echo $iList['phone'];?>"></td>
                ...
                <div role="tabpanel" class="tab-pane active" id="home">
                  <div class="ab-1">
                    <form action="" method="POST">
                     ...
                     <Br>
                      <button type="submit" name="staffSMS"style="float:left;">SEND SMS </button>
                      </div>
				...
				$aryList=$db->getRows("select * from staff_manage where create_by_userid='".$create_by_userid."'");
						foreach($aryList as $iList)
							{	$i=$i+1;
							 ...
							 ?>
                          <tr>
                            <td><input type="checkbox" name="sendSsmsToStaff[]" value="<?php echo $iList['phone']; ?>"></td>
