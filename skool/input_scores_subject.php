 ...
 * Version: 2.0 (PHP 8.x Compatible) - Auto-Load Enabled
 * ============================================================================
 */
...
$FileName = 'input_score_subject_teacher.php';
...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .class-item { padding: 15px 20px; border-bottom: 1px solid #f0f0f0; cursor: pointer; transition: all 0.2s; text-decoration: none; display: block; }
        .class-item:hover { background: #f8f9ff; }
        .class-item.active { background: #1B3058; color: white; }
        .class-item.active small { color: rgba(255,255,255,0.7); }
        .class-item small { display: block; font-size: 11px; color: #999; margin-top: 5px; }
        ...
        .subject-badge { display: inline-block; padding: 2px 8px; border-radius: 12px; font-size: 10px; font-weight: 600; margin-top: 5px; }
        .subject-badge-primary { background: #1B3058; color: white; }
                    ...
                    <div class="class-list-panel">
                        ...
                        <div class="class-list">
                            <?php if (!empty($teacherSubjects)): ?>
                                ...
                                ?>
                                    <a href="?action=input_score&randomid=<?= urlencode($subject['randomid']) ?><?= $selectedSession ? '&session='.urlencode($selectedSession) : '' ?><?= $selectedTerm ? '&term_id='.urlencode($selectedTerm) : '' ?><?= $selectedAssessment ? '&assesment='.urlencode($selectedAssessment) : '' ?>" class="class-item <?= ($randomid == $subject['randomid']) ? 'active' : '' ?>">
                                ...
                                <?php endforeach; ?>
                            <?php else: ?>
                                <div class="class-item">No subjects assigned to you</div>
                            <?php endif; ?>
                        </div>
                    </div>
                            ...
                            <div class="filter-card">
                                <form method="GET" action="" id="filterForm">
                                    ...
                                    <div class="auto-load-status">
                                        ...
                                        <?php else: ?>
                                            <i class="fa fa-info-circle"></i> Select session and term to load students automatically.
                                        <?php endif; ?>
                                    </div>
                                </form>
                            </div>
                                ...
                                <form method="POST" id="scoreForm">
                                    ...
                                    <div style="background: #fff; border-radius: 16px; box-shadow: 0 4px 20px rgba(0,0,0,0.05); overflow: hidden;">
                                        <div class="panel-header">
                                            ...
                                            <span style="float:right; font-size:12px; font-weight:normal;">Enter scores for all assessments below and click Save All</span>
                                        </div>
...
function validateAndSubmit() {
    ...
    document.getElementById('scoreForm').appendChild(studentIdsInput);
    ...
    document.getElementById('scoreForm').appendChild(assessmentIdsInput);
    ...
    var btn = document.querySelector('button[onclick="validateAndSubmit()"]');
    if (btn) {
...
if (!document.querySelector('#spinner-style')) {
    ...
    style.textContent = '.spinner { display: inline-block; width: 16px; height: 16px; border: 2px solid #fff; border-top-color: transparent; border-radius: 50%; animation: spin 0.8s linear infinite; } @keyframes spin { to { transform: rotate(360deg); } }';
