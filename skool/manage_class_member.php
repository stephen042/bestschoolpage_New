...
// ===== FIXED: Initialize variables to prevent warnings =====
$stat = [];
$create_by_userid = $_SESSION['userid'] ?? 0;

// ===== FIXED: Check if $_GET variables exist =====
$randomid = $_GET['randomid'] ?? '';
$new_randomid = $_GET['new_randomid'] ?? '';
$action = $_GET['action'] ?? '';

// ===== FIXED: Check if records exist before using them =====
$studetail = [];
if(!empty($randomid)) {
    $studetail = $db->getRow("select * from school_class where randomid='".$randomid."' and create_by_userid='".$create_by_userid."'");
    if(!$studetail) $studetail = [];
}

$newStudetail = [];
if(!empty($new_randomid)) {
    $newStudetail = $db->getRow("select * from school_class where randomid='".$new_randomid."' and create_by_userid='".$create_by_userid."'");
    if(!$newStudetail) $newStudetail = [];
}

if(isset($_POST['update_class_id']))
{
    // ===== FIXED: Check if class_id exists in POST =====
    if(isset($_POST['class_id']) && is_array($_POST['class_id']) && !empty($_POST['class_id']))
    {
        foreach($_POST['class_id'] as $key=>$val)
        {
            $aryData = array(	
                'class' => $studetail['id'] ?? 0,    
            );  
            $flgIn1 = $db->updateAry("manage_student", $aryData, "where id='".$val."'");
        }
        $stat['success'] = "Students moved successfully!";
    }
    else
    {
        $stat['error'] = "Please select at least one student.";
    }
}
                                ...
                                <tbody> 
                                    ...
                                    if(!empty($classDetail)) {
                                        foreach($classDetail as $iList) { ?>
                                            <tr>
                                                <td><?php echo $i++; ?></td> 
                                                <td><?php echo $db->getVal("select session from school_session where id='".($iList['session_id'] ?? 0)."'"); ?></td> 
                                        ...
                                        <tr>
                                            <td colspan="4" style="text-align:center">No classes found. Please create a class first.</td>
                                        </tr>
                                    <?php } ?>
                                </tbody>
                            </table>
                                        ...
                                        <div class="zasw">
                                            <div class="zawq Wizard-a1">
                                                <table id="example" class="display">
                                                    <tbody>
                                                        <?php 
                                                        $aryDetail = $db->getRows("select * from school_class where randomid!='".$randomid."' and create_by_userid='".$create_by_userid."' ");
                                                        if(!empty($aryDetail)) {
                                                            foreach ($aryDetail as $iList) { ?>
                                                                <tr>
                                                                    <td style="padding:0px;"></td>
                                                                    <td class="sectsab <?php echo ($new_randomid == ($iList['randomid'] ?? '')) ? "active" : ""; ?>">
                                                                        <a href="<?php echo $Filename; ?>?action=classes&randomid=<?php echo $randomid; ?>&new_randomid=<?php echo $iList['randomid'] ?? ''; ?>">
                                                                            <ul>
                                                                                <li>
                                                                                    <span class="zwq"><i class="fa fa-book" style="font-size:48px"></i></span>
                                                                                    <span class="subject">
                                                                                        <?php echo $iList['name'] ?? ''; ?><br>
                                                                                        <?php echo $db->getVal("select session from school_session where id = '" . ($iList['session_id'] ?? 0) . "'"); ?>
                                                                                    </span>
                                                                                </li>
                                                                            </ul>
                                                                        </a>
                                                            ...
                                                            <tr>
                                                                <td colspan="2" style="text-align:center">No other classes available</td>
                                                            </tr>
                                                        <?php } ?>
                                                    </tbody>
                                                    <tfoot>
                                    ...
                                    <div class="zasw1">
                                        <div class="col-md-9">
                                            <table class="table table-bordered">
                                                ...
                                                <tr>
                                                    <th>Session: <?php echo $db->getVal("select session from school_session where id='".($studetail['session_id'] ?? 0)."'"); ?></th>
                                                </tr>
                                                <tr>
                                                    <th>Class: <?php echo $db->getVal("select name from school_class where id='".($studetail['id'] ?? 0)."'"); ?></th>
                                                </tr>
                                            </table>
                                                                        ...
                                                                        <div class="card-box">
                                                                            <table class="table table-striped table-bordered">
                                                                                ...
                                                                                <tbody>
                                                                                    ...
                                                                                    $i=1; 
                                                                                    $newtudentDetail = [];
                                                                                    if(!empty($newStudetail)) {
                                                                                        $newtudentDetail = $db->getRows("select * from manage_student where class='".($newStudetail['id'] ?? 0)."' and session='".($newStudetail['session_id'] ?? 0)."' and create_by_userid='".$create_by_userid."'");
                                                                                    }
                                                                                    if(!empty($newtudentDetail)) {
                                                                                        ...
                                                                                        <tr>
                                                                                            <td colspan="7" style="text-align:center">No students found in this class</td>
                                                                                        </tr>
                                                                                    <?php } ?>
                                                                                </tbody>
                                                                            </table>
                                                                        </div>
                                                                        <button style="float:right" type="submit" name="update_class_id" class="gwt-Button">Update Class</button>
