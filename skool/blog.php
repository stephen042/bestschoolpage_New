...
{ 
	...
	$validate->addRule($_POST['short_description'],'','Short Description',true);
	...
	if($validate->validate() && count($stat)==0)
	{
		...
		$iLastId=$db->getVal("select id from school_blog order by id desc")+1;		
		$randomId=randomFix(15).'-'.$iLastId;
		$iPageUrl = PageUrl($_POST['title']).'-'.$iLastId;
		...
		$aryData=array(	
			...
			$flgIn1 = $db->insertAry("school_blog",$aryData);
...
{ 
	...
	{ 
		...
		$aryData=array(	
			...
			$flgIn = $db->updateAry("school_blog", $aryData , "where randomid='".$_GET['randomid']."' ");
...
{
	$flgIn1 = $db->delete("school_blog","where randomid='".$_GET['randomid']."' ");			
	...
	<div class="container">
        ...
        <div class="row">
            ...
            <section>
				...
				<div class="form-group clearfix">
					...
					<div class="col-lg-10">
						<textarea class="form-control" name="short_description"><?php echo $_POST['short_description']; ?></textarea>
					</div>
				</div>
			...
			$aryDetail=$db->getRow("select * from  school_blog where randomid='".$_GET['randomid']."'");	
			?>
            <div class="card-box">
			<form role="form" action="" method="post" enctype="multipart/form-data">
			<section>
			...
			$GetEmailId=$db->getRow("select * from  school_blog where randomid='".$_GET['randomid']."'");
			?>
            <div class="card-box">
            <section>
				<div class="form-group clearfix">
				...
				<div class="form-group clearfix">
					<label class="col-lg-2 control-label " for="userName">Short Description :</label>
					<?php echo $GetEmailId['short_description']; ?>
				</div>
				...
				<div class="form-group clearfix">
					<label class="col-lg-2 control-label " for="userName">Date :</label>
					<?php echo $GetEmailId['date']; ?>
				</div>
            ...
            <table id="datatable" class="table table-striped table-bordered">
              ...
              <tbody>
                <?php
		$aryList=$db->getRows("select * from school_blog where create_by_userid='".$create_by_userid."' order by id desc");
		...
		$aryPgAct["id"]=$iList['id'];
			...
			<tr>
                ...
                <td><?php echo $iList['short_description']; ?></td>
