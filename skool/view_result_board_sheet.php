...
/**
 * ============================================================================
 * VIEW RESULT BROAD SHEET - MODERN REDESIGN
 * ============================================================================
 * Description: View comprehensive broad sheet with all subjects and assessments
...
$PageTitle = "View Broad Sheet";
$FileName = 'view_result_board_sheet.php';
...
$postAssessmentsStr = implode(',', $postAssessments);
...
if (!empty($classDetail['id']) && $postSession > 0 && $postTerm > 0 && !empty($postAssessments) && !empty($subjects)) {
	...
	foreach ($students as $student) {
		...
		foreach ($subjects as $subject) {
			// Get total score for this subject across all selected assessments
			$score = db_get_val(
				"SELECT SUM(score) FROM input_score_class_teacher 
                 WHERE assesment_id IN ($postAssessmentsStr) 
                 AND student_id = ? 
                 AND subject_id = ? 
                 AND session_id = ? 
                 AND term_id = ? 
...
$assessmentIdsStr = implode('-', $postAssessments);
?>
<!DOCTYPE html>
<html>
			...
			<div class="content">
				<div class="broad-container">
					...
					<div class="page-header">
						...
						<p>View comprehensive broad sheet with all subjects and assessments</p>
					</div>
						...
						<div class="main-panel">
							...
							<?php if (!empty($classDetail)): ?>
								...
								<div class="filter-card">
									...
									<!-- Print Button -->
									<div class="btn-group">
										<?php if (empty($postAssessments)): ?>
											<button type="button" class="btn btn-danger" onclick="alert('Please select at least one assessment first')">
												<i class="fa fa-file-pdf-o"></i> Print Broad Sheet
											</button>
										<?php else: ?>
											<a href="<?= SKOOL_URL ?>class_teacher_broadsheet_pdf.php?randomid=<?= urlencode($_GET['randomid'] ?? '') ?>&assesment=<?= urlencode($assessmentIdsStr) ?>&session=<?= $postSession ?>&term_id=<?= $postTerm ?>"
								...
								<!-- Results Display -->
								<?php if (!empty($postSession) && !empty($postTerm) && !empty($postAssessments) && !empty($subjects)): ?>
										...
										<div class="result-card">
											<div class="card-header">
												<div class="title">
													...
													<?= htmlspecialchars($classDetail['name'] ?? 'Class') ?> - Broad Sheet
												</div>
												<div class="subtitle">
											...
											<div class="summary-stats">
												...
												<div class="summary-stat">
													<div class="number"><?= number_format($highestAverage, 2) ?></div>
													<div class="label">Highest</div>
												</div>
												<div class="summary-stat">
													<div class="number"><?= number_format($lowestAverage, 2) ?></div>
								...
								<?php elseif (!empty($postSession) && !empty($postTerm) && empty($postAssessments)): ?>
									<div class="alert alert-info">
										<i class="fa fa-info-circle"></i>
										<div>Please select at least one assessment to view the broad sheet.</div>
									</div>
								...
								<?php elseif (empty($postSession)): ?>
									<div class="alert alert-info">
										<i class="fa fa-info-circle"></i>
										<div>Please select a session, term, and assessments to view the broad sheet.</div>
									</div>
								<?php endif; ?>
							...
							<?php else: ?>
								<div class="alert alert-info">
									<i class="fa fa-info-circle"></i>
									<div>Please select a class from the left panel to view the broad sheet.</div>
								</div>
							<?php endif; ?>
		...
		document.addEventListener('DOMContentLoaded', function() {
			...
			selects.forEach(function(select) {
				select.addEventListener('change', function() {
					...
					if (session && session.value && term && term.value && assessments.length > 0) {
						form.submit();
					}
				});
			});
