 ...
 * - Can remove a child from a parent (detach)
 * - Parent accounts are stored in student_guardian table
 * ============================================================================
 */
...
// ============================================================================
// HELPER: Safe htmlspecialchars for PHP 8.x
// ============================================================================
function hsc($string)
{
...
// ============================================================================
// GET LINKED CHILDREN FOR A PARENT (WITH DISTINCT)
// ============================================================================
function getLinkedChildren($parentId)
{
    return db_get_rows(
        "SELECT DISTINCT student_id, first_name, last_name, email, phone, gender, 
         date_of_birth, date_of_admission, session, term_id, class, picture, 
         randomid, parent_id, create_by_userid 
         FROM manage_student 
         WHERE parent_id = ? 
         GROUP BY student_id 
...
// ============================================================================
// REMOVE CHILD FROM PARENT (DETACH)
// ============================================================================
if (isset($_GET['remove_child']) && !empty($_GET['child_id']) && !empty($_GET['parent_id'])) {
    $childId = $_GET['child_id'];
    $parentId = $_GET['parent_id'];

    // Verify the child belongs to this parent
    $child = db_get_row(
        "SELECT student_id, parent_id FROM manage_student WHERE student_id = ? AND parent_id = ? AND create_by_userid = ?",
        [$childId, $parentId, $create_by_userid]
    );

    if ($child) {
        // Remove the parent_id from the child
        $result = db_update(
            "manage_student",
            ['parent_id' => ''],
            "student_id = ? AND create_by_userid = ?",
            [$childId, $create_by_userid]
        );

        if ($result !== false) {
            $_SESSION['success'] = "Child " . hsc($childId) . " has been removed from this parent.";
        } else {
            $_SESSION['error'] = "Failed to remove child from parent.";
        }
    } else {
        $_SESSION['error'] = "Child not found or does not belong to this parent.";
    }
...
if (isset($_GET['reset_password']) && !empty($_GET['parent_id'])) {
    ...
    // Update password in student_guardian table
    $result = db_update("student_guardian", ['password' => $hashedPassword], "parent_id = ?", [$parentId]);
...
if (isset($_GET['toggle_status']) && !empty($_GET['parent_id'])) {
    $parentId = $_GET['parent_id'];
    $currentStatus = db_get_val("SELECT status FROM student_guardian WHERE parent_id = ?", [$parentId]);
...
// ============================================================================
// DELETE PARENT (Only if no children linked)
// ============================================================================
if (isset($_GET['delete']) && !empty($_GET['parent_id'])) {
    $parentId = $_GET['parent_id'];

    // Check if parent has linked children
    $childCount = db_get_val("SELECT COUNT(DISTINCT student_id) FROM manage_student WHERE parent_id = ?", [$parentId]);

    if ($childCount > 0) {
        $_SESSION['error'] = "Cannot delete parent. This parent has $childCount linked child(ren). Remove the children first or reassign them.";
    } else {
...
// ============================================================================
$parents = db_get_rows(
    "SELECT DISTINCT ms.parent_id, sg.*
     FROM (SELECT DISTINCT parent_id FROM manage_student WHERE create_by_userid = ? AND parent_id IS NOT NULL AND parent_id != '') ms
     LEFT JOIN student_guardian sg ON ms.parent_id = sg.parent_id AND sg.type = 1
...
$editParentChildren = [];
if (!empty($randomid)) {
    $editParent = db_get_row("SELECT * FROM student_guardian WHERE randomid = ? AND type = 1", [$randomid]);
    if ($editParent) {
...
<head>
    ...
    <style>
        * {
        ...
        .parent-card.active .parent-name,
        .parent-card.active .parent-id {
            color: white;
        }
        ...
        /* Remove child button hover effect */
        .remove-child-btn {
            color: #fde0e3;
            border-color: #dc3545;
        }
            ...
            <div class="content">
                <div class="parents-container">
                    ...
                    <div class="parents-main">
                        <?php if ($editParent):
                            $parentStatus = $editParent['status'] ?? '1';
                                        ...
                                        <div class="form-group"><label>State</label>
                                            <select name="home_state" class="form-select">
                                                ...
                                                <?php foreach ($states as $s): ?>
                                                    <option value="<?= hsc($s['id'] ?? '') ?>" <?= (($editParent['home_state'] ?? '') == ($s['id'] ?? '')) ? 'selected' : '' ?>><?= hsc($s['title'] ?? '') ?></option>
                                                <?php endforeach; ?>
                                            </select>
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-grid">
                                        ...
                                        <div class="form-group"><label>State</label>
                                            <select name="office_state" class="form-select">
                                                <option value="">Select State</option>
                                                <?php foreach ($states as $s): ?>
                                                    <option value="<?= hsc($s['id'] ?? '') ?>" <?= (($editParent['office_state'] ?? '') == ($s['id'] ?? '')) ? 'selected' : '' ?>><?= hsc($s['title'] ?? '') ?></option>
                                                <?php endforeach; ?>
                                            </select>
                                        </div>
                                    </div>

                                    <!-- Linked Children Section with Remove Button -->
                                    <div class="children-section">
                                        <h4><i class="fa fa-child"></i> Linked Children (<?= count($editParentChildren) ?> unique children)</h4>
                                            ...
                                            <?php foreach ($editParentChildren as $child): ?>
                                                <div class="child-card">
                                                    ...
                                                    <div class="child-actions">
                                                        ...
                                                        <a href="<?= $FileName ?>?remove_child=1&child_id=<?= urlencode($child['student_id'] ?? '') ?>&parent_id=<?= urlencode($editParent['parent_id'] ?? '') ?>&randomid=<?= urlencode($randomid) ?>"
                                                            class="btn btn-danger btn-sm remove-child-btn"
                                                            onclick="return confirm('Remove <?= hsc(($child['first_name'] ?? '') . ' ' . ($child['last_name'] ?? '')) ?> from this parent? This will remove the parent link but keep the student record.')">
